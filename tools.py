...
def generate_images_cgan(generator, path, h, w, c, latent_dim, std, nr, nc, iteration):
    figure = np.zeros((h * nr, w * nc, c))
    for ri in range(nr):
        for ci in range(nc):
...
def generate_images_cvaegan(generator, path, h, w, c, latent_dim, nr, nc, iteration):
    figure = np.zeros((h * nr, w * nc, c))
    for ri in range(nr):
        for ci in range(nc):

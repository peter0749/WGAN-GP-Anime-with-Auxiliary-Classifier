...
def Fitness(img, decoder):
    def bce(x):
        y_t, y_p = np.clip(img[np.newaxis,...]*.5+.5,1e-5,1-1e-5), np.clip(decoder.predict(x)*.5+.5,1e-5,1-1e-5)
...
for i in range(args.runs):
    ...
    y_t, y_p = np.clip(img*.5+.5,1e-5,1-1e-5), np.clip(img_reconstruct*.5+.5,1e-5,1-1e-5)
